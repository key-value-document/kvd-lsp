...
fn locate_path(text: &str, path: &str) -> Range {
...
fn path_at_lines(lines: &[&str], line_idx: usize) -> Vec<String> {
...
fn key_before_colon(content: &str) -> Option<String> {
...
fn collect_keys(node: &kvd_rs::value::Node, prefix: &str, out: &mut Vec<String>) {
    match node {
...
fn find_path_line(text: &str, path: &[String]) -> Option<u32> {
    let lines: Vec<&str> = text.lines().collect();
    for (i, _) in lines.iter().enumerate() {
...
mod tests {
    ...
    fn word_at_finds_key() {
        let w = word_at("port: 8080\n", Position { line: 0, character: 1 });
        assert_eq!(w.as_deref(), Some("port"));
    }

    #[test]
    fn word_at_returns_none_on_punctuation() {
        assert!(word_at("port: 8080\n", Position { line: 0, character: 4 }).is_none());
    }
    ...
    fn collect_keys_skips_metakeys() {
        let doc = kvd_rs::deserialize::from_str(
            "port: 8080\n__schema__:\n  port:\n    type: int\n",
        )
        .unwrap();
        let mut keys = Vec::new();
